...
__host__ void hex_to_bytes(const char *hex_str, uint8_t *bytes_out,
                           size_t num_bytes) {
...
__device__ void hash_half_fast(const char *candidate,
...
__device__ void indexToCandidate(uint64_t index, int len, char *out_str) {
  ...
  for (int i = len - 1; i >= 0; i--) {
    out_str[i] =
        FULL_CHARSET[index % CHARSET_SIZE]; // Carattere in base alla posizione
                                            // corrente del thread
...
*/
__global__ void crackHalfKernel(const uint8_t target_bytes[8],
                                char *cracked_out, uint64_t start_index,
                                uint64_t total_work, unsigned int candidate_len,
  ...
  uint64_t idx = start_index +
                 gid; // indice assoluto nel range [start_index, total_work-1]
  ...
  if (*reinterpret_cast<const uint64_t *>(candidate_block) ==
      *reinterpret_cast<const uint64_t *>(target_bytes)) {
...
__host__ void splitHash(const char *hash_hex, uint8_t target1[8],
                        uint8_t target2[8]) {
  hex_to_bytes(hash_hex, target1, 8);
  hex_to_bytes(hash_hex + 16, target2, 8);
}

__host__ bool checkIfFound(unsigned int *foundFlag1, unsigned int *foundFlag2,
                           unsigned int *h_flag1, unsigned int *h_flag2) {
...
__host__ void cleanGPU(uint8_t *target1GPU, uint8_t *target2GPU,
                       char *crackedPassword1GPU, char *crackedPassword2GPU,
...
int main(int argc, char **argv) {
  ...
  */
  ...
  const uint8_t HOST_NULL_HALF[8] = {0xAA, 0xD3, 0xB4, 0x35,
                                     0xB5, 0x14, 0x04, 0xEE};
  ...
  if (memcmp(target1, HOST_NULL_HALF, 8) == 0) {
    ...
    cleanGPU(target1GPU, target2GPU, crackedPassword1GPU, crackedPassword2GPU,
             foundFlag1, foundFlag2);
    return 0;
  }
  ...
  for (unsigned int len = 1; len <= 7; len++) {
    ...
    printf(
        "[DEBUG] Raggiunta lunghezza %u, combinazioni totali da testare %lu\n",
    ...
    // hardware
    uint64_t maxBlocksPerLaunch =
        (uint64_t)maxGridDimX; // già ottenuto prima del for
    ...
    while (start < totalCombinations) {
      ...
      uint64_t blocks =
          (workThisLaunch + threadsPerBlock - 1) / threadsPerBlock;
      ...
      if (h_flag1 == 0) {
        ...
        crackHalfKernel<<<blocks, threadsPerBlock, 0, stream1>>>(
            target1GPU, crackedPassword1GPU, start, totalCombinations, len,
            foundFlag1);
      ...
      // kernel
      if (h_flag2 == 0) {
        crackHalfKernel<<<blocks, threadsPerBlock, 0, stream2>>>(
            target2GPU, crackedPassword2GPU, start, totalCombinations, len,
            foundFlag2);
  ...
  char crackedFirstPart[9] = {0},
       crackedSecondPart[9] = {0}; // 15 per il null byte delle stringhe in c
  cudaMemcpy(crackedFirstPart, crackedPassword1GPU, halfBytes,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(crackedSecondPart, crackedPassword2GPU, halfBytes,
             cudaMemcpyDeviceToHost);
