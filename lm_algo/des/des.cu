...
__constant__ const uint8_t SHIFTS[16] = {1, 1, 2, 2, 2, 2, 2, 2,
                                         1, 2, 2, 2, 2, 2, 2, 1};
...
__device__ __forceinline__ uint64_t permute(uint64_t input,
                                            const uint8_t *table, int out_bits,
                                            int in_bits) {
  uint64_t out = 0;
...
__device__ __forceinline__ void generate_subkeys(uint64_t key64,
                                                 uint64_t subkeys[16]) {
...
mangler_cipher_function(uint32_t r_block, uint64_t bit_48_key) {
  ...
  uint64_t temp_xored = xored_plaintext; // se xored_plaintext non e' riusato
                                         // possiamo eliminarlo successivamente
#pragma unroll
  for (int i = 7; i >= 0; i--) { // big-endian itero al contrario
    chunks[i] =
        (uint8_t)(temp_xored & 0x3F); // prendo gli ultimi 6 bit meno
...
#pragma unroll
  for (int i = 0; i < 8; i++) { // ogni S-Box da applicare ad ogni chunk
    ...
    uint8_t row =
        ((chunks[i] & 0x20) >> 4) |
        (chunks[i] &
         0x1); // lo shift serve per spostarlo a destra di 4 posti dato che si
               // trova in posizione e poi lo concateno con l'or all'altro,
               // avendolo shiftato solo di 4 ottengo 000010 con un posto a
               // destra per concatenare l'altro
    // suppongo che chunks[i] sia 101010
    ...
    uint8_t column = (chunks[i] >> 1) &
                     0x0F; // 0x0F e' (0000 1111) cosi' shiftando elimino quello
    ...
    combined_chunks =
        (combined_chunks << 4) |
        val; // sposto di 4 i precedenti poi mergio con i successivi
  }
...
__device__ __forceinline__ void des_encrypt_block(const uint8_t key_bytes[8],
                                                  const uint8_t input_block[8],
...
#pragma unroll
  ...
  */
  ...
  // Metà DESTRA (32 bit meno significativi)
  uint32_t R_prev = (uint32_t)
      permuted_block; // se faccio << 32 ottengo tutti 0 perche, conversione a
                      // uint32_t taglia automaticamente la meta' di sinistra
                      // prende usa little-endian come precedenza

#pragma unroll
  for (int i = 0; i < 16;
       i++) { // feistel cicle, devono essere 16 round esatti non 15! bug!
    uint32_t f_result =
        mangler_cipher_function(R_prev, subkeys[i]); // ritorna 32 bit
  ...
  /*
     ...
     formata mettendo R[16] a sinistra (bit più significativi) e L[16] a destra
     (bit meno significativi)
  */
  // 32 bits di shift sono
  uint64_t pre_output =
      ((uint64_t)R_prev << 32) |
      L_prev; // R[16] va prima convertito a 64 per poterlo shiftare di 32
...
#pragma unroll
  for (int i = 7; i >= 0; i--) {
    output_block[i] =
        (uint8_t)reverse_initial_permutation; // prendo gli 8 bit meno
                                              // significativi troncandoli
    reverse_initial_permutation =
        reverse_initial_permutation >>
        8; // shifto a destra di 8, distruggo reverse_initial_permutation tanto
           // non mi serve piu'
  }
}